...
(function(process) {
  ...
  (function(root, factory) {
    ...
    (function(H) {
      ...
      H.Tick.prototype = {
        addLabel: function() {
          ...
          if (!defined(label)) {
            tick.label = label = defined(str) && labelOptions.enabled ? chart.renderer.text(str, 0, 0, labelOptions.useHTML).add(axis.labelGroup) : null;
            if (label) {
      ...
      Highcharts.Legend.prototype = {
        ...
        renderTitle: function() {
          ...
          if (titleOptions.text) {
            if (!this.title) {
              this.title = this.chart.renderer.label(titleOptions.text, padding - 3, padding - 4, null, null, null, options.useHTML, null, 'legend-title').attr({zIndex: 1}).add(this.group);
            }
        ...
        drawGraph: function() {
          ...
          each(props, function(prop) {
            ...
            if (area) {
              ...
              area = series[areaKey] = series.chart.renderer.path(areaPath).addClass(prop[1]).attr({zIndex: 0}).add(series.group);
              area.isArea = true;
            }
    ...
    (function(H) {
      var pick = H.pick,
          seriesType = H.seriesType;
      seriesType('spline', 'line', {}, {getPointSpline: function(points, point, i) {
          var smoothing = 1.5,
              denom = smoothing + 1,
              plotX = point.plotX,
              plotY = point.plotY,
    ...
    (function(H) {
      ...
      seriesType('bar', 'column', null, {inverted: true});
    }(Highcharts));
    (function(H) {
      var Series = H.Series,
          seriesType = H.seriesType;
    ...
    (function(H) {
      ...
      seriesType('ohlc', 'column', {
        lineWidth: 1,
        tooltip: {pointFormat: '<span class="highcharts-color-{point.colorIndex}">\u25CF</span> <b> {series.name}</b><br/>' + 'Open: {point.open}<br/>' + 'High: {point.high}<br/>' + 'Low: {point.low}<br/>' + 'Close: {point.close}<br/>'},
        threshold: null,
        stickyTracking: true
      }, {
        directTouch: false,
    ...
    (function(H) {
      ...
      seriesType('candlestick', 'ohlc', merge(defaultPlotOptions.column, candlestickOptions), {drawPoints: function() {
          var series = this,
              points = series.points,
              chart = series.chart;
          each(points, function(point) {
