  ...
  (function(factory) {
    ...
    (function(H) {
      ...
      extend(Pane.prototype, {
        ...
        renderBackground: function(backgroundOptions, i) {
          ...
          this.background[i][method]({'d': this.axis.getPlotBandPath(backgroundOptions.from, backgroundOptions.to, backgroundOptions)}).attr({'class': 'highcharts-pane ' + (backgroundOptions.className || '')});
        },
        defaultOptions: {
          center: ['50%', '50%'],
          size: '85%',
      ...
      if (!H.radialAxisExtended) {
        ...
        radialAxisMixin = {
          ...
          beforeSetTickPositions: function() {
            this.autoConnect = (this.isCircular && pick(this.userMax, this.options.max) === undefined && this.endAngleRad - this.startAngleRad === 2 * Math.PI);
            if (this.autoConnect) {
          ...
          getPosition: function(value, length) {
            return this.postTranslate(this.isCircular ? this.translate(value) : this.angleRad, pick(this.isCircular ? length : this.translate(value), this.center[2] / 2) - this.offset);
          },
    ...
    (function(H) {
      ...
      seriesType('arearange', 'area', {
        threshold: null,
        tooltip: {pointFormat: '<span class="highcharts-color-{series.colorIndex}">\u25CF</span> {series.name}: <b>{point.low}</b> - <b>{point.high}</b><br/>'},
        trackByArea: true,
        dataLabels: {
          align: null,
          verticalAlign: null,
        ...
        drawPoints: function() {
          ...
          while (i < pointLength) {
            ...
            if (!series.chart.polar) {
              point.isInside = point.isTopInside = (point.plotY !== undefined && point.plotY >= 0 && point.plotY <= series.yAxis.len && point.plotX >= 0 && point.plotX <= series.xAxis.len);
            }
            i++;
          }
          seriesProto.drawPoints.apply(series, arguments);
    ...
    (function(H) {
      var seriesType = H.seriesType,
          seriesTypes = H.seriesTypes;
      seriesType('areasplinerange', 'arearange', null, {getPointSpline: seriesTypes.spline.prototype.getPointSpline});
    }(Highcharts));
    (function(H) {
      ...
      seriesType('gauge', 'line', {
        ...
        drawPoints: function() {
          ...
          if (pivot) {
            ...
            series.pivot = renderer.circle(0, 0, pick(pivotOptions.radius, 5)).attr({zIndex: 2}).addClass('highcharts-pivot').translate(center[0], center[1]).add(series.group);
          }
        },
    ...
    (function(H) {
      ...
      seriesType('boxplot', 'column', {
        threshold: null,
        tooltip: {pointFormat: '<span class="highcharts-color-{point.colorIndex}">' + '\u25CF</span> <b> {series.name}</b><br/>' + 'Maximum: {point.high}<br/>' + 'Upper quartile: {point.q3}<br/>' + 'Median: {point.median}<br/>' + 'Lower quartile: {point.q1}<br/>' + 'Minimum: {point.low}<br/>'},
        whiskerLength: '50%'
      }, {
        ...
        drawPoints: function() {
          ...
          each(points, function(point) {
            ...
            if (point.plotY !== undefined) {
              ...
              point.stem[verb]({d: ['M', crispX, q3Plot, 'L', crispX, highPlot, 'M', crispX, q1Plot, 'L', crispX, lowPlot]});
              if (doQuartiles) {
                ...
                left += crispCorr;
                right += crispCorr;
                point.box[verb]({d: ['M', left, q3Plot, 'L', left, q1Plot, 'L', right, q1Plot, 'L', right, q3Plot, 'L', left, q3Plot, 'z']});
              }
              if (whiskerLength) {
                ...
                point.whiskers[verb]({d: ['M', crispX - pointWiskerLength, highPlot, 'L', crispX + pointWiskerLength, highPlot, 'M', crispX - pointWiskerLength, lowPlot, 'L', crispX + pointWiskerLength, lowPlot]});
              }
              medianPlot = Math.round(point.medianPlot);
              crispCorr = (point.medianShape.strokeWidth() % 2) / 2;
              medianPlot = medianPlot + crispCorr;
              point.medianShape[verb]({d: ['M', left, medianPlot, 'L', right, medianPlot]});
            }
          });
        },
        setStackedPoints: noop
    ...
    (function(H) {
      var correctFloat = H.correctFloat,
          ...
          seriesType = H.seriesType,
          seriesTypes = H.seriesTypes;
      seriesType('waterfall', 'column', {dataLabels: {inside: true}}, {
        pointValKey: 'y',
        translate: function() {
          var series = this,
              options = series.options,
          ...
          for (i = 0, len = points.length; i < len; i++) {
            ...
            range = pick(stack && stack[point.x].points[stackIndicator.key], [0, yValue]);
            if (point.isSum) {
            ...
            if (point.isSum) {
              ...
              shapeArgs.height = Math.min(yAxis.translate(range[0], 0, 1, 0, 1), yAxis.len) - shapeArgs.y;
            } else if (point.isIntermediateSum) {
              shapeArgs.y = yAxis.translate(range[1], 0, 1, 0, 1);
